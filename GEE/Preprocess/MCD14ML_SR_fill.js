...
// (back-fill for months with no fires)
// ==============================================
// @author: Tianjia Liu
...
for (var iYear = 0; iYear < xYears.length; iYear++) {
  ...
  for (var iMonth = 0; iMonth < xMonths.length; iMonth++) {
    var inMonth = xMonths[iMonth];
    ...
    for (var inDay = 1; inDay <= days_of_month[inMonth-1]; inDay++) {
      ...
      var mxd09ga_dayFilter = ee.ImageCollection('MODIS/006/' + satMODISsr)
        .filter(ee.Filter.calendarRange(inYear,inYear,'year'))
        .filter(ee.Filter.calendarRange(inMonth,inMonth,'month'))
        .filter(ee.Filter.calendarRange(inDay,inDay,'day_of_month'));
      
      if (mxd09ga_dayFilter.size().getInfo() > 0) {
        var mxd09ga_day = ee.Image(mxd09ga_dayFilter.first())
        ...
        var fire_sr_incm = mxd09ga_day.select('sur_refl_b01').clamp(0,1)
          .reduceRegions({
            ...
            crs: mxd09ga_day.projection(),
            scale: mxd09ga_day.projection().nominalScale()
          });
        ...
        fire_sr_month[inDay-1] = ee.FeatureCollection([ee.Feature(null)]);
        fire_sr_area_month[inDay-1] = ee.Feature(null,{YYYYMMDD: yyyymmdd});
      }
    }
     
    fire_sr_month = ee.FeatureCollection(fire_sr_month).flatten()
      .filter(ee.Filter.gte('FRP',0));
