# --------------------------------------------
# Adjusted FRP for Tier 3 states - Rajasthan
...
timestamp(prefix=paste(outState,": ##------ "))
...
for (iYear in xYears) {
  ...
  for (inSat in c("Aqua","Terra")) {
    ...
    # read MODIS FRP
    modisFRPgrid <- read.csv(paste0("gee/MxD14A1/MxD14A1_FRP_",outState,".csv"))
    ...
    for (iMonth in xMonths) {
      ...
      if (iYear >= 2012) {
        # read VIIRS FRP
        viirs_frp <- read.csv(paste0("gee/VNP14IMGML_FRPboost/VNP14IMGML_FRP_",outState,"_",
      ...
      fire_sr_area_name <- paste0("gee/MCD14ML_SR/MCD14ML_SR_Area_",inSat,"_",outState,
    ...
    # account for possible duplicate observations: 0.97 scaling factor
  ...
  write.table(fireMat,paste0("tables/MODISadjFRP/MODISadjFRP_",outState,"_",iYear,".csv"),sep=",",row.names=F)
